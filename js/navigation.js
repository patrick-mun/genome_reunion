...
// Slide courant — exposé globalement pour les onclick dans le HTML
window.current = 0;
let current = window.current;
 
/**
 * Navigue vers la slide n.
 * Exposée globalement pour les onclick du HTML.
 */
window.go = function go(n) {
  n = Math.max(0, Math.min(TOTAL - 1, n));
 
  slides[current].classList.remove('on');
  current = n;
  window.current = n;  // sync pour les onclick inline du HTML
