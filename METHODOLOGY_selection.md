...
#### **2.1.1 Variante recommandée — noyau géographique + pool libre**

La version de base impose une stratification géographique stricte sur la totalité des 350 WGS. Cette stratégie maximise la représentativité démographique, mais peut sous-capturer certains profils génétiques très informatifs présents dans des secteurs minoritaires.

**Variante recommandée pour analyse de sensibilité :**
- **90–95% des WGS** alloués strictement par secteur (`315–332` individus)
- **5–10% des WGS** conservés dans un **pool libre insulaire** (`18–35` individus), sélectionnés sur l'ensemble de la cohorte selon S_div global, sous contrainte IBD

**Justification** :
- maintient la représentativité géographique comme architecture dominante ;
- autorise l'entrée de profils hautement informatifs à l'échelle de l'île ;
- réduit le risque de sur-rigidité démographique ;
- fournit un test empirique : comparer les performances `stratification stricte` vs `stratification + pool libre`.

**Recommandation opérationnelle** : conserver la version stricte comme **référence principale** et documenter la version avec pool libre comme **analyse de sensibilité secondaire**.
...
#### **3.3.1 Robustesse de l'algorithme greedy — multi-ordre / multi-seed**
...
| 6 | Benchmark vs alternatives — métriques + version pool libre | Tableau métriques réel |
| 7 | Analyse sensibilité des poids + LOCO + stabilité multi-ordre | Rapport sensibilité/robustesse |
