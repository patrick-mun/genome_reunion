...
Une sélection aléatoire des 350 WGS serait simple, mais risquerait de manquer des profils génétiques rares ou fondateurs. À l’inverse, une sélection uniquement basée sur les profils extrêmes maximiserait la diversité capturée, mais biaiserait fortement la représentativité. La méthode V3 cherche donc un compromis explicite :
...
Ce réglage peut être ajusté en analyse de sensibilité : 95/5, 92/8, 90/10.
...
La V3 introduit donc un score distinct :
...
Cette répartition est paramétrable et sera testée en validation.
...
La sélection individuelle pure peut sélectionner plusieurs individus proches dans l’espace PCA/ADMIX. Pour limiter cette redondance non capturée par l’IBD, la V3 recommande d’ajouter un terme de nouveauté marginale, au moins en analyse de sensibilité.
...
La V3 doit produire au moins deux listes :

1. `selection_stricte_350` : sélection géographique stricte, sans bras découverte.
2. `selection_hybride_V3_350` : noyau géographique + bras découverte contrôlé.

Une troisième liste peut être produite :

3. `selection_hybride_V3_consensus` : individus stables sur multi-ordres/multi-seeds.
...
Objectif : produire un phasage mieux adapté aux haplotypes réunionnais.

### 12.2 Phasage familial
...
3. maximin IBD ;
4. S_div naïf ;
5. S_div stratifié strict ;
6. S_div hybride V3 ;
7. S_div hybride V3 + gain marginal.
...
et mesurer la variation :

- couverture allélique ;
- KS ;
- divergence ancestrale ;
- individus remplacés ;
- performance d’imputation.

Interprétation :

- faible variance : poids robustes ;
- forte variance : calibration critique à optimiser avant déploiement.
...
## 19. Annexe A — Commandes indicatives PLINK / ADMIXTURE

Les commandes ci-dessous sont indicatives. Elles devront être adaptées au format exact des fichiers, au build génomique, à la puce utilisée et au logiciel de phasage retenu.
...
Le seuil recommandé dans la V3 est :
...
Le phasage familial dépendra du logiciel retenu. Le fichier de pedigree devra être versionné, contrôlé et séparé des fichiers populationnels afin d’éviter toute confusion entre ressource de phasage et cohorte indépendante.
...
Les références sur phasage familial, imputation et LAI devront être complétées dans la version destinée à publication.
...
| V2 | Révision renforcée | Panel hybride, bras découverte, QC, ROH fondateur, puce→WGS |
| **V3** | Reformulation consolidée | Familles nucléaires hors cohorte, puce 1,9 M SNP, phasage Réunion, S_div global distinct, IBD KING clarifié, quotas intégrés, recalibrage fréquences formalisé |

---

## 23. Conclusion méthodologique

La V3 formalise une architecture en trois piliers :
...
- amélioration du phasage par information familiale ;
